...
def _count_parses(monkeypatch) -> list[str]:
    """Which logs the listing reads from disk, through the session index."""
    parsed: list[str] = []
    real_build, real_extend = SessionIndex._build, SessionIndex._extend
    ...
    monkeypatch.setattr(SessionIndex, "_build", build)
    monkeypatch.setattr(SessionIndex, "_extend", extend)
    return parsed
