...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="quickcode-packaged-") as scratch:
        ...
        # An offline endpoint guarantees that smoke checks never use a real key.
        config = root / ".quickcode"
        config.mkdir()
        (config / "config.json").write_text(json.dumps({
            "update_check": False,
            "profiles": {"default": {"base_url": "http://127.0.0.1:1/v1", "api_key_env": "QC_SMOKE_NO_KEY"}},
        }), encoding="utf-8")
        ...
        with (root / "server.log").open("w", encoding="utf-8") as log:
            ...
            try:
                ...
                for path in ["/api/bootstrap", "/api/projects", "/api/credits"]:
                ...
                print(f"Frozen QuickCode {expected}: version, health, authenticated routes, workspace assets and cache headers passed.")
            finally:
                process.terminate()
                try:
