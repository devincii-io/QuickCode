...
quickcode/
  ...
  server/
    ...
    jobs_api.py           # .../conversations/{id}/jobs: list, tail and kill background shell jobs
  session/
  ...
  tools/
    ...
    fs/                   # textfile.py (encodings, line endings, staleness), walk.py, patterns.py,
                          # diffpreview.py (the capped diff a permission prompt shows)
...
- Prompt choices: **allow once · always allow (persist rules) · deny with message** (deny text returns as the tool result so the model adapts). "Always allow" writes one exact rule per subcommand that asked (`PermissionEngine.suggest_rules`), and the prompt lists them, with the diff an edit or write would make and a *Why?* that asks the engine about the pending call.
