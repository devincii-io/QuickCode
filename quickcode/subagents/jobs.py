...
def _attr(text: str) -> str:
    """One value, safe inside a double-quoted XML attribute."""
    return (
        str(text)
        .replace("&", "&amp;")
        .replace("<", "&lt;")
...
class JobRecord:
    ...
    def to_tag(self) -> str:
        """The one-line handle the model sees, in every tool that reports one."""
        parts = [
            f'id="{_attr(self.agent_id)}"',
            f'type="{_attr(self.agent_type)}"',
            f'status="{self.status}"',
            f'seconds="{self.seconds()}"',
        ]
        if self.status in TERMINAL:
            parts.append(f'collected="{str(self.collected).lower()}"')
        if self.description:
            parts.append(f'description="{_attr(self.description)}"')
        return "<agent_job " + " ".join(parts) + "/>"
